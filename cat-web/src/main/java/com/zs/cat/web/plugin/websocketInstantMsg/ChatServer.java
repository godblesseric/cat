package com.zs.cat.web.plugin.websocketInstantMsg;
...
public class ChatServer extends WebSocketServer {
    ...
    public void onMessage(WebSocket conn, String message) {
        ...
        if (null != message && message.startsWith("zsadminqq313596790")) {
            this.userjoin(message.replaceFirst("zsadminqq313596790", ""), conn);
        }
        if (null != message && message.startsWith("Leavezsadminqq313596790")) {
            this.userLeave(conn);
        }
        if (null != message && message.contains("zsadmin886")) {
            String toUser = message.substring(message.indexOf("zsadmin886") + 10, message.indexOf("zszsadmin888"));
            message = message.substring(0, message.indexOf("zsadmin886")) + "[私信]  " + message.substring(message.indexOf("zszsadmin888") + 12, message.length());
